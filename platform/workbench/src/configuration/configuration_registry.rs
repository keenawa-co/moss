...
impl ConfigurationSchemaStorage {
    ...
    #[allow(unused)]
    fn remove_from_schema(&mut self, key: &str, property: &ConfigurationPropertySchema) {
        self.all_settings_schema.remove(key);

        match property.scope {
            ConfigurationScope::Platform => {
                self.platform_settings_schema.remove(key);
            }
            ConfigurationScope::Machine => {
                self.machine_settings_schema.remove(key);
            }
            ConfigurationScope::Window => {
                self.window_settings_schema.remove(key);
            }
            ConfigurationScope::Resource => {
                self.resource_settings_schema.remove(key);
            }
        }
    }
}
...
pub struct ConfigurationRegistry {
    ...
    excluded_configuration_properties: HashMap<String, ConfigurationPropertySchema>,
}
...
impl ConfigurationRegistry {
    ...
    pub fn deregister_configuration(&mut self, configuration: &ConfigurationNode) {
        for key in configuration.properties.keys() {
            self.configuration_properties.remove(key);
            self.configuration_schema_storage
                .remove_from_schema(key, &configuration.properties[key]);
