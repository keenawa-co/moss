...
import { IFrameworkPart, PanelInitParameters, PanelUpdateEvent, Parameters } from "../panel/types";
...
export abstract class PaneviewPanel extends BasePanelView<PaneviewPanelApiImpl> implements IPaneview, IPaneviewPanel {
  private readonly _onDidChangeExpansionState: Emitter<boolean> = new Emitter<boolean>({ replay: true });
  ...
  constructor(
    ...
    this.addDisposables(
      ...
      addDisposableListener(this.element, "mouseenter", (ev: MouseEvent) => {
        this.api._onMouseEnter.fire(ev);
      }),
      addDisposableListener(this.element, "mouseleave", (ev: MouseEvent) => {
  ...
  layout(size: number, orthogonalSize: number): void {
    ...
    const [width, height] =
      this.orientation === Orientation.HORIZONTAL ? [size, orthogonalSize] : [orthogonalSize, size];
    if (this.isExpanded()) {
