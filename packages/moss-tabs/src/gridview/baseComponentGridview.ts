...
export abstract class BaseGrid<T extends IGridPanelView> extends Resizable implements IBaseGrid<T> {
  ...
  readonly onDidMaximizedChange: Event<MaximizedChanged<T>> = this._onDidMaximizedChange.event;

  private readonly _onDidActiveChange = new Emitter<T | undefined>();
  readonly onDidActiveChange: Event<T | undefined> = this._onDidActiveChange.event;

  protected readonly _bufferOnDidLayoutChange = new AsapEvent();
  readonly onDidLayoutChange: Event<void> = this._bufferOnDidLayoutChange.onEvent;

  private readonly _onDidViewVisibilityChangeMicroTaskQueue = new AsapEvent();
  readonly onDidViewVisibilityChangeMicroTaskQueue = this._onDidViewVisibilityChangeMicroTaskQueue.onEvent;
  ...
  constructor(container: HTMLElement, options: BaseGridOptions) {
    ...
    this.addDisposables(
      ...
      this.gridview.onDidViewVisibilityChange(() => this._onDidViewVisibilityChangeMicroTaskQueue.fire()),
  ...
  protected doAddGroup(group: T, location: number[] = [0], size?: number): void {
    this.gridview.addView(group, size ?? Sizing.Distribute, location);

    this._onDidAdd.fire(group);
  }

  protected doRemoveGroup(group: T, options?: { skipActive?: boolean; skipDispose?: boolean }): T {
