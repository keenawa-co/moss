...
export class Paneview extends CompositeDisposable implements IDisposable {
  ...
  constructor(container: HTMLElement, options: { orientation: Orientation; descriptor?: ISplitViewDescriptor }) {
    super();
  ...
  public addPane(pane: PaneviewPanel, size?: number | Sizing, index = this.splitview.length, skipLayout = false): void {
  ...
  public removePane(index: number, options: { skipDispose: boolean } = { skipDispose: false }): PaneItem {
