...
export class Overlay extends CompositeDisposable {
  ...
  setupDrag(dragTarget: HTMLElement, options: { inDragMode: boolean } = { inDragMode: false }): void {
          ...
          addDisposableWindowListener(window, "pointermove", (e) => {
            ...
            const moveTop = () => {
              top = clamp(
                y,
                -Number.MAX_VALUE,
                startPosition!.originalY + startPosition!.originalHeight > containerRect.height
                  ? this.getMinimumHeight(containerRect.height)
                  : Math.max(0, startPosition!.originalY + startPosition!.originalHeight - Overlay.MINIMUM_HEIGHT)
              );
            ...
            const moveBottom = () => {
              ...
              height = clamp(
                y - top,
                top < 0 && typeof this.options.minimumInViewportHeight === "number"
                  ? -top + this.options.minimumInViewportHeight
            ...
            const moveLeft = () => {
              left = clamp(
                x,
                -Number.MAX_VALUE,
                startPosition!.originalX + startPosition!.originalWidth > containerRect.width
                  ? this.getMinimumWidth(containerRect.width)
                  : Math.max(0, startPosition!.originalX + startPosition!.originalWidth - Overlay.MINIMUM_WIDTH)
              );
            ...
            const moveRight = () => {
              ...
              width = clamp(
                x - left,
                left < 0 && typeof this.options.minimumInViewportWidth === "number"
                  ? -left + this.options.minimumInViewportWidth
