...
describe("droptarget", () => {
  ...
  test("that dragover events are marked", () => {
    ...
    expect((event as any)["__dockview_droptarget_event_is_used__"]).toBeTruthy();
  });
  ...
  test("directionToPosition", () => {
    ...
    expect(() => directionToPosition("bad_input" as any)).toThrow("invalid direction 'bad_input'");
  });
  ...
  test("positionToDirection", () => {
    ...
    expect(() => positionToDirection("bad_input" as any)).toThrow("invalid position 'bad_input'");
  });
  ...
  test("default", () => {
    ...
    check(element.getElementsByClassName("dv-drop-target-selection").item(0) as HTMLDivElement, {
      top: "0px",
      left: "0px",
      width: "50%",
      height: "100%",
    ...
    fireEvent(target, createOffsetDragOverEvent({ clientX: 40, clientY: 19 }));
    ...
    fireEvent(target, createOffsetDragOverEvent({ clientX: 160, clientY: 81 }));
    ...
    fireEvent(target, createOffsetDragOverEvent({ clientX: 161, clientY: 0 }));
    ...
    fireEvent(target, createOffsetDragOverEvent({ clientX: 100, clientY: 50 }));
    expect(droptarget.state).toBe("center");
    expect((element.getElementsByClassName("dv-drop-target-selection").item(0) as HTMLDivElement).style.transform).toBe(
      ""
    );
  ...
  describe("calculateQuadrantAsPercentage", () => {
    test("variety of cases", () => {
      ...
      for (const input of inputs) {
        expect(calculateQuadrantAsPercentage(new Set(input.directions), input.x, input.y, 100, 100, 20)).toBe(
          input.result
        );
      }
    });
  ...
  describe("calculateQuadrantAsPixels", () => {
    test("variety of cases", () => {
      ...
      for (const input of inputs) {
        expect(calculateQuadrantAsPixels(new Set(input.directions), input.x, input.y, 100, 100, 20)).toBe(input.result);
      }
    });
  });
});
