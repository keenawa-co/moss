...
import { ISplitviewPanelProps, SplitviewReact, SplitviewReadyEvent } from "../../splitview/splitview";
import { setMockRefElement } from "../__test_utils__/utils";

describe("splitview react", () => {
  let components: Record<string, React.FunctionComponent<ISplitviewPanelProps>>;
  ...
  test("default", () => {
    ...
    render(<SplitviewReact orientation={Orientation.VERTICAL} components={components} onReady={onReady} />);
  ...
  test("that the component can update parameters", async () => {
    ...
    const wrapper = render(
      <SplitviewReact orientation={Orientation.VERTICAL} components={components} onReady={onReady} />
    );
