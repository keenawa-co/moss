...
describe("dockviewGroupPanelModel", () => {
  ...
  test("toJSON() default", () => {
    const dockviewComponent = new DockviewComponent(document.createElement("div"), {
      createComponent(options) {
        switch (options.name) {
          case "component":
            return new TestContentPart(options.id);
    ...
    const cut = new DockviewGroupPanelModel(document.createElement("div"), dockviewComponent, "id", {}, null as any);
  ...
  test("that openPanel with skipSetActive doesn't set panel to active", () => {
    ...
    const cut = new DockviewGroupPanelModel(groupviewContainer, dockviewComponent, "id", {}, null as any);
    const contentContainer = groupviewContainer.getElementsByClassName("dv-content-container").item(0)!.childNodes;
  ...
  test("that the .locked behaviour is as", () => {
    ...
    function run(value: number) {
      fireEvent.dragEnter(element);
      fireEvent(element, createOffsetDragOverEvent({ clientX: value, clientY: value }));
    }
  ...
  test("that watermark is added", () => {
    ...
    expect(container.getElementsByClassName("watermark-test-container").length).toBe(0);
    expect(container.getElementsByClassName("dv-tabs-and-actions-container").length).toBe(1);
