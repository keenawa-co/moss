...
impl From<InstantiationType> for ServiceInstantiationMode {
    fn from(value: InstantiationType) -> Self {
        match value {
            InstantiationType::Eager => ServiceInstantiationMode::Active,
...
struct ServiceCollectionState {
    services: FnvHashMap<TypeId, Arc<dyn AnyService>>,
    pending_services: FnvHashMap<TypeId, Box<dyn FnOnce(&AppHandle) -> Arc<dyn AnyService>>>,
...
impl ServiceCollection {
    ...
    pub fn get<T: AnyService>(&self) -> Result<ServiceHandle<T>> {
        let type_id = TypeId::of::<T>();
    ...
    pub fn get_unchecked<T: AnyService>(&self) -> ServiceHandle<T> {
        let type_id = TypeId::of::<T>();
        let service_metadata = self
            .state
            .write()
