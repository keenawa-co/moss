...
fn batch_spawn_entities(world: &mut Frame, n: usize) {
...
fn system_integrate_motion(world: &mut Frame, query: &mut PreparedQuery<(&mut Position, &Speed)>) {
    let mut rng = thread_rng();
...
fn system_fire_at_closest(world: &mut Frame) {
...
fn system_remove_dead(world: &mut Frame) {
...
fn print_world_state(world: &mut Frame) {
    println!("\nEntity stats:");
