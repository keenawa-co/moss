...
#[test]
fn simple_test() {
    let mut world = Frame::new();
}

#[test]
fn random_access() {
    let mut world = Frame::new();
    let e = world.spawn((Tooltip("abc"), Order(123)));
    let f = world.spawn((Tooltip("def"), Order(456), true));

    assert_eq!(*world.get::<&Tooltip>(e).unwrap(), Tooltip("abc"));
    assert_eq!(*world.get::<&Order>(e).unwrap(), Order(123));

    let r = world.entity(e).unwrap().entity().id();

    // dbg!(r.get::<&Tooltip>().unwrap());
...
fn spawn_buffered_entity() {
    ...
    buffer.insert(ent1, (13, 7.11, "crater"));
    ...
    assert_eq!(*world.get::<&&str>(ent1).unwrap(), "crater");
...
fn remove_buffered_component() {
    let mut world = Frame::new();
    let mut buffer = CommandBuffer::new();
    let ent = world.spawn((7, true, "crater"));
