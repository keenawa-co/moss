...
impl<'w, Q: Query> QueryBorrow<'w, Q> {
    pub(crate) fn new(world: &'w Frame) -> Self {
        Self {
            world,
            borrowed: false,
            _marker: PhantomData,
...
impl<'q, Q: Query> QueryIter<'q, Q> {
    ...
    unsafe fn new(world: &'q Frame) -> Self {
...
impl<'q, Q: Query> QueryMut<'q, Q> {
    pub(crate) fn new(world: &'q mut Frame) -> Self {
        assert_borrow::<Q>();
...
impl<Q: Query> PreparedQuery<Q> {
    ...
    pub fn query<'q>(&'q mut self, world: &'q Frame) -> PreparedQueryBorrow<'q, Q> {
    ...
    pub fn query_mut<'q>(&'q mut self, world: &'q mut Frame) -> PreparedQueryIter<'q, Q> {
        assert_borrow::<Q>();
    ...
    pub fn view_mut<'q>(&'q mut self, world: &'q mut Frame) -> PreparedView<'q, Q> {
        assert_borrow::<Q>();
