...
fn gen_dynamic_bundle_impl(
    ...
    quote! {
        unsafe impl #impl_generics ::crater::DynamicBundleClone for #ident #ty_generics #where_clause {
            #[allow(clippy::forget_copy)]
            unsafe fn put_with_clone(mut self, mut f: impl ::core::ops::FnMut(*mut u8, ::crater::TypeInfo, ::crater::DynamicClone)) {
                #(
                    f(
                        (&mut self.#field_members as *mut #tys).cast::<u8>(),
                        ::crater::TypeInfo::of::<#tys>(),
                        ::crater::DynamicClone::new::<#tys>()
                    );
