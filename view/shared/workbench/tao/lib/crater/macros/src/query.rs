...
pub fn derive(input: DeriveInput) -> Result<TokenStream2> {
    ...
    let fetches = queries
        .iter()
        .map(|ty| quote! { <#ty as ::crater::Query>::Fetch })
        .collect::<Vec<_>>();
    ...
    let state = match data.fields {
        syn::Fields::Named(_) => quote! {
            ...
            #vis struct #state_ident {
                #(
                    #fields: <#fetches as ::crater::Fetch>::State,
                )*
            }
        },
        syn::Fields::Unnamed(_) => quote! {
            #[derive(Clone, Copy)]
            #vis struct #state_ident(#(<#fetches as ::crater::Fetch>::State),*);
        },
        syn::Fields::Unit => quote! {
            #[derive(Clone, Copy)]
            #vis struct #state_ident;
    ...
    Ok(quote! {
        const _: () = {
            ...
            impl<'a> ::crater::Query for #ident<'a> {
                type Item<'q> = #ident<'q>;
                ...
                unsafe fn get<'q>(fetch: &Self::Fetch, n: usize) -> Self::Item<'q> {
                    #(
                        let #intermediates: <#queries as ::crater::Query>::Item<'q> = <#queries as ::crater::Query>::get(&fetch.#fields, n);
                    )*
            ...
            unsafe impl ::crater::Fetch for #fetch_ident {
                ...
                fn access(archetype: &::crater::Archetype) -> ::core::option::Option<::crater::Access> {
                    let mut access = ::crater::Access::Iterate;
                    #(
                ...
                #[allow(unused_variables)]
                fn borrow(archetype: &::crater::Archetype, state: Self::State) {
                    #(#fetches::borrow(archetype, state.#fields);)*
                }

                #[allow(unused_variables)]
                fn prepare(archetype: &::crater::Archetype) -> ::core::option::Option<Self::State> {
                ...
                #[allow(unused_variables)]
                fn execute(archetype: &::crater::Archetype, state: Self::State) -> Self {
                    Self {
                        #(
                ...
                #[allow(unused_variables)]
                fn release(archetype: &::crater::Archetype, state: Self::State) {
                ...
                fn for_each_borrow(mut f: impl ::core::ops::FnMut(::core::any::TypeId, bool)) {
                    #(
                        <#fetches as ::crater::Fetch>::for_each_borrow(&mut f);
                    )*
                }
            }
        };
