...
const StatusBar = ({ branch }: { branch?: string }) => {
  return (
    <div className="flex w-full items-center px-5 bg-olive-900 text-stone-50 text-xs ">
      <div className="grow">
      ...
      {branch ? (
        <div>
          <StatusBarButton className="group flex">
            <svg
...
function StatusBarButton({ children, className }: ComponentPropsWithoutRef<'button'>) {
  return (
    <button
      className={twMerge(
        'flex  items-center gap-2 grow hover:bg-white hover:bg-opacity-10 focus:bg-white focus:bg-opacity-10 transition px-2.5 py-0.5',
        className
      )}
    >
      {children}
