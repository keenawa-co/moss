...
const buttonRootStyles = cva(
  "relative flex items-center cursor-pointer justify-center rounded-lg border-2  transition hover:brightness-110 active:brightness-90",
  {
    variants: {
      ...
      variant: {
        solid: `   background-(--bg-solid) border-(--border-solid) text-(--text-solid)`,
        outlined: `background-(--bg-outlined) border-(--border-outlined) text-(--text-outlined)`,
        soft: `    background-(--bg-soft) border-(--border-soft) text-(--text-soft)`,
        ghost: `   background-transparent border-transparent hover:background-(--bg-ghost) hover:border-(--border-ghost) text-(--text-ghost) `,
      },
      size: {
        "xs": "h-7",
        "sm": "h-8",
