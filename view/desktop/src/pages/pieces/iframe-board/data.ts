...
export function isColumnDropTarget(data: Record<string | symbol, unknown>): data is TColumnDropTarget {
...
export function getCardDropTarget({ cardId, columnId }: { cardId: string; columnId: string }): TCardDropTarget {
  return {
    cardId,
    columnId,
    [cardDropTargetKey]: true,
...
export function getDroppedExternalCardId({ source }: { source: ExternalDragPayload }): string | null {
  if (!isAndroid()) {
