...
import { subtaskGap, subtaskIconWidth, subtaskIdWidth, subtaskInlinePadding } from "../primitives/linear/constants";
import { Subtask, type SubtaskAppearance, SubtaskPreview, type SubtaskProps } from "../primitives/linear/subtask";
...
function DragPreview({ subtaskRef, children, ...props }: SubtaskProps & { subtaskRef: RefObject<HTMLElement> }) {
  ...
  useEffect(() => {
    ...
    function getAdjustment(location: DragLocationHistory) {
      ...
      let initialXFromPreview = location.initial.input.clientX - (initialOffset.x + previewRect.width / 2);
      ...
      let initialYFromPreview = location.initial.input.clientY - (initialOffset.y + previewRect.height / 2);
      ...
      onDrag({ location }) {
        ...
        if (isFirstTick) {
          ...
          Object.assign(previewElement.style, {
            transform: `translate(${offsetX + initialAdjustment.x}px, ${offsetY + initialAdjustment.y}px)`,
            opacity: 1,
          });
