...
function DraggableSubtask({ index, id, data, reorderItem, ...subtaskProps }: DraggableSubtaskProps) {
  ...
  const { dragState, isHovering, closestEdge } = useSortableField({
    ...
    onGenerateDragPreview({ nativeSetDragImage }) {
      return setCustomNativeDragPreview({
        ...
        render({ container }) {
          ReactDOM.render(<DraggableSubtaskPreview>{subtaskProps.title}</DraggableSubtaskPreview>, container);
          return () => {
        ...
        <span css={elementBeforeButtonContainerStyles}>
          <DropdownMenu
            trigger={({ triggerRef, ...triggerProps }) => (
              <DragHandleButton ref={mergeRefs([setDragHandle, triggerRef])} {...triggerProps} label="reorder" />
            )}
          >
            <DropdownItemGroup>
