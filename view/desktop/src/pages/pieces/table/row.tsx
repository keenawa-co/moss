...
export const Row = memo(function Row({
  ...
  useEffect(() => {
    ...
    return combine(
      ...
      dropTargetForElements({
        element,
        canDrop({ source }) {
          return source.data.instanceId === instanceId && source.data.type === "item-row" && source.data.item !== item;
        },
  ...
  return (
    <Fragment>
          ...
          <td key={property} css={textOverflowStyles}>
            {
               ...
               */
              columnIndex === 0 && <RowMenuButton ref={dragHandleRef} rowIndex={index} amountOfRows={amountOfRows} />
            }
      ...
      {state.type === "preview" ? createPortal(<Preview item={item} properties={properties} />, state.container) : null}
    </Fragment>
  );
});
