...
function getColumnType({ index, amountOfHeaders }: { index: number; amountOfHeaders: number }): ColumnType {
  ...
  return (
    <Fragment>
      <th ref={ref} css={[thStyles, state.type === "dragging" ? headerDraggingStyles : undefined]}>
        ...
        <ColumnMenuButton ref={dragHandleRef} columnIndex={index} amountOfHeaders={amountOfHeaders} />

        {/* Resizer */}
        {renderResizeHandle ? (
          <div ref={resizerRef} css={[resizerStyles, state.type === "resizing" ? resizingStyles : undefined]}></div>
        ) : null}
      ...
      {state.type === "preview" ? ReactDOM.createPortal(<ColumnPreview property={property} />, state.container) : null}
    </Fragment>
  );
}
...
function ColumnPreview({ property }: { property: keyof Item }) {
  ...
  return (
    <Box backgroundColor="elevation.surface" padding="space.100" paddingBlockStart="space.0" xcss={previewStyles}>
