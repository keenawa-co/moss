 ...
 * @jsx jsx
 */
import { forwardRef, Fragment, memo, type Ref, useCallback, useEffect, useRef, useState } from "react";
  ...
  return (
    ...
    <div onClick={onClick}>
      <Stack
        ref={ref}
        testId={`item-${key}`}
        xcss={[containerStyles, state === draggingState && draggingStyles, isSelected && selectedStyles]}
        space="space.200"
      >
          ...
          <Box xcss={actionStyles} style={{ marginInline: -8, marginBlock: -4 }}>
            <DropdownMenu
              trigger={({ triggerRef, ...triggerProps }) => (
                <Button
                  ref={actionMenuTriggerRef ? mergeRefs([triggerRef, actionMenuTriggerRef]) : triggerRef}
                  iconBefore={<MoreIcon spacing="spacious" color="currentColor" label={`Move ${name}`} />}
                  appearance="subtle"
                  {...triggerProps}
                />
              )}
        ...
        <Inline spread="space-between" alignBlock="center">
          ...
          <Inline space="space.050" alignBlock="center">
            ...
            <Avatar size="small">
              {(props) => <div {...props} ref={props.ref as React.Ref<HTMLDivElement>} css={noPointerEventsStyles} />}
            </Avatar>
          </Inline>
        </Inline>
