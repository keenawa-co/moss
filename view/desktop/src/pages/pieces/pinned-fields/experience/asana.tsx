...
function DragPreview({ fieldRef, children }: { children: ReactNode; fieldRef: RefObject<HTMLElement> }) {
...
function DraggableField({ index, item }: DraggableFieldProps) {
  ...
  return (
    <Field
      ref={ref}
      icon={isHovering ? <DragHandlerIcon color="currentColor" spacing="spacious" label="" /> : item.icon}
    >
      {item.label}
